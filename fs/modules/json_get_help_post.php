...
if(!isset($_SESSION)){
	...
	if (isset ($_SESSION['userHelpYou']))
	{
		...
		if (count($listIDUser)> $start)
		{
			...
			for ($ii=$start + 1;$ii<count($listIDUser);$ii++)
			{
				...
				$ridposts=mysqli_query($con,"select post_id from atw_post where post_iduser=".$listIDUser[$ii]." and post_group=".$idPostGroup." and type_invalid=1 ORDER BY post_id DESC limit 2,1");		
	...
	if ($listIDUser!="" && $qidUser!="")
	{
		...
		// $ridposts=mysqli_query($con,"select up_post_idPost from atw_post,fs_up_post where ($qidUser)  and post_group=".$idPostGroup." and up_post_idPost=post_id and type_invalid=1 group by post_iduser ORDER BY up_post_id DESC");
		$ridposts=mysqli_query($con,"select up_post_idPost from atw_post,fs_up_post where  post_group=".$idPostGroup." and up_post_idPost=post_id and type_invalid=1 group by post_iduser ORDER BY up_post_id DESC");
		...
		if ($qidPost!="")
		{
			...
			$result_post=mysqli_query($con,"select * from atw_post,atw_user  where post_iduser=user_id and (".$qidPost.") and type_invalid=1 and post_group=".$idPostGroup );
			while (($row = mysqli_fetch_array($result_post)) && ($countpost <3))
			{
		...
		$ridposts=mysqli_query($con,"select max(post_id) postId from atw_post where $qidUser  and post_group=".$idPostGroup." and type_invalid=1 group by post_iduser ORDER BY post_id DESC");
		$qidPost="";
		if ($ridposts->num_rows>0)
		{
			$row = mysqli_fetch_array($ridposts);
	...
	{
			$result_post=mysqli_query($con,"select * from atw_post,atw_user  where post_iduser=user_id and post_group=".$idPostGroup." and type_invalid=1 ORDER BY post_id DESC limit 100" );
			if ($IdPostDisPlay!="")
				$result_post=mysqli_query($con,"select * from atw_post,atw_user  where post_iduser=user_id and ".$IdPostDisPlay." and post_group=".$idPostGroup." and type_invalid=1 ORDER BY post_id DESC limit 100" );
