...
if(!isset($_SESSION)){
	...
	//echo "select * from atw_post,atw_user  where post_iduser=user_id and post_id<".$idPostStart." ORDER BY post_id DESC limit 5";
	$result_post=mysqli_query($con,"select * from atw_post,atw_user  where post_iduser=user_id and post_id<".$idPostStart." ORDER BY post_id DESC limit 5" );
	$countPost=0;
	$countPostAvailable=0;
	while ($row = mysqli_fetch_array($result_post))
	{		
		if ($countPost%4==0 && $countPost!=$countPostAvailable)
		{
			$inavalble=$countPost - $countPostAvailable;
			//echo " Invalable ".$inavalble. " ". $countPost." ".$countPostAvailable;
			$con=mysqli_connect($host,$user,$pass,$db);
			mysqli_set_charset($con, "utf8");
			$result_post=mysqli_query($con,"select * from atw_post where post_id<".$row['post_id']." ORDER BY post_id DESC limit 5" );
			$countPost=$countPostAvailable;
		}
		if (checkAvailableLinks($row['post_url'],$row['post_iduser']) && checkAvailableLinks($row['post_full_url'],$row['post_iduser']))
		{
			$countPostAvailable++;	
			$idPost= $row['post_id'];						
			...
			if ($user_id==$_SESSION['session-user'])
			{
