...
if(!isset($_SESSION)){
	...
	if(isset($_POST['idPostNotify'])){
		...
		$result_post=mysqli_query($con,"select * from atw_post,atw_point where post_iduser=idUser and post_id!=$idPostNotify  GROUP BY post_iduser ORDER BY point DESC limit 5 " );
	}else
		$result_post=mysqli_query($con,"select * from atw_post,atw_user,atw_point where post_iduser=idUser and     post_iduser=user_id and post_group=".$idgroup." GROUP BY post_iduser ORDER BY point DESC limit 100 ");	
	$countPost=0;
	...
	while (($row = mysqli_fetch_array($result_post)) && ($countPostAvailable<5))
	{
		...
		if ($countPost%4==0 && $countPost!=$countPostAvailable)
		{
			$con=mysqli_connect($host,$user,$pass,$db);
			mysqli_set_charset($con, "utf8");
			$result_post=mysqli_query($con,"select * from atw_post,atw_point where post_id<".$row['post_id']." and post_group=".$idgroup." GROUP BY post_iduser ORDER BY point DESC limit 5" );
