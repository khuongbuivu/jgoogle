<?php
	...
	$resultpost=mysqli_query($con,"Select post_id FROM atw_post where type_invalid=1");
	echo $resultpost->num_rows;
	$numdel=$resultpost->num_rows - 300;	
	$listIds=mysqli_query($con,"SELECT post_id FROM atw_post  where post_id>1 and type_invalid=1 order by post_id desc limit 10,".$numdel);
