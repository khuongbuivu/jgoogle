...
$result=mysqli_query($con,'select * from atw_user limit 50');	
while ($row = mysqli_fetch_array($result))
{
	$leveluser=getLevelUser($row['user_id'],$row['user_time_join']);
	if ($leveluser==0)
		echo '<li class="als-item"><a href="https://www.facebook.com/profile.php?id='.$row['user_id'].'"><img src="https://graph.facebook.com/'.$row['user_id'].'/picture" alt="'.$row['user_name'].'" title="'.$row['user_name'].'" /></a><div style="position:absolute; top: -3px;left:-4px"><img src="images/css/new.png"></div></li>';
	else
