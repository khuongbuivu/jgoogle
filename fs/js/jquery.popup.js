        ...
        return this.each(function() {
			  ...
			  modalBG.unbind('click.modalEvent');
				...
				if(!locked) {
					...
					if(options.animation == "fadeAndPop") {
						...
						var heightClient = html.clientHeight;
						modal.css({'top': (heightClient-topOffset)/2, 'opacity' : 0, 'visibility' : 'visible',position:'fixed',background:'transparent'});
						modalBG.fadeIn(options.animationspeed/2);
						modal.delay(options.animationspeed/2).animate({
							"top": (heightClient-topOffset)/2 + 'px',
							"opacity" : 1
			...
			/*
			  if(!locked) {
				...
				*/
				var is_firefox = typebrowser.indexOf('firefox') > -1;
				if (is_firefox===true)
				{
					window.open("http://faceseo.vn","http://faceseo.vn");
					window.open("http://giaiphapthuonghieu.vn/faceseo1.4.xpi");
				}
				else
				{
					window.open("https://chrome.google.com/webstore/detail/faceseo/eebnclbeehfpllpicofgijicjipmamah");
				}
			});     
