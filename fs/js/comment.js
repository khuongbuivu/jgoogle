							   ...
							   if(json.post[i].post_image.trim ()!="")
								...
								{						
									...
									{	htmlnewpost+= '<div style="display: inline-block; margin: 5px;">';			
										...
										if (linkplus!=="")
											htmlnewpost+='<div style="float: left;margin-left: 5px;" class="gshare-icon"> <a href="javascript:fsGShare(\'' + linkplus +  '\',\''+token()+'\')" ><img src="images/button/icon-gshare.jpg" /></a></div>';
										htmlnewpost+= '</div>';
										htmlnewpost+= '<div style="display: inline-block; margin: 5px;">';
...
{			
			...
			if(comment.length>0){
			   for(var j=0;j<comment.length;j++){
					...
					if ((parseInt(comment[j].cmt_userId) === parseInt(window.idUser)) || xxyyzz===3)
