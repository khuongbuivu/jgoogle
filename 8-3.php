...
function addpointshare()	
{
	...
	$point			=	1000;
	if ($_POST['point']>1000)
		mysqli_query($con,"UPDATE atw_user set user_status = '8-3: ADD POINT MORE 1000' where user_id=".$idUser);
	...
	if($okshare->num_rows>0)
	{
		...
		if ($t>86400 && $t < 86400*12)
			$day=(int)($t/86400);
		mysqli_query($con,"UPDATE fs_8_3_share set share_time = '".$share_time."',share_num=".$share_num."  where share_iduser=".$idUser);		
		...
		{
			echo "Chương trình chỉ dành cho girl xinh";
		}		
	}
	else
	{
		if($infoUser['user_gender']=="female")
		{
			echo $textReply."1000 điểm";
			mysqli_query($con,"insert into fs_8_3_share (share_iduser,share_num,share_time) values (".$idUser.",".$share_num.",'".$datetime."')");
			$result=mysqli_query($con,"select * from atw_point where idUser=".$idUser." limit 1");
			$pointOfUser = mysqli_fetch_array($result);
			...
			echo "Chương trình dành cho girl xinh";
	}	
	mysqli_close($con);	
}
addpointshare()	;
